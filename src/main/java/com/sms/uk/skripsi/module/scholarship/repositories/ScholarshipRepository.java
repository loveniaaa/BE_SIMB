...
public interface ScholarshipRepository extends JpaRepository<Scholarship, String>, JpaSpecificationExecutor<Scholarship> {
    ...
    @Modifying
    @Query("UPDATE Scholarship s SET s.isDeleted = :isDeleted WHERE s.user.uuid = :uuid")
    void updateIsDeletedByUserId(@Param("uuid") String uuid, @Param("isDeleted") boolean isDeleted);
