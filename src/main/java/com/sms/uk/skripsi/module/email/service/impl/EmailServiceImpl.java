...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    public void sendEmailNotificationDocument(String email, String name, List<String> message) throws MessagingException {
        try {
            log.info("Mengirim email verifikasi dokumen ke: {}", email);

            // Load logo
            byte[] imageBytes = Files.readAllBytes(Paths.get(EmailConstant.TEMPLATE_IMAGE_PATH + "logo-uk.png"));
            String base64Image = Base64.getEncoder().encodeToString(imageBytes);

            // Siapkan context template
            Context context = new Context(LocaleContextHolder.getLocale());
            context.setVariable("name", name);
            context.setVariable("logoBase64", base64Image);
            context.setVariable("invalidDocs", message);

            String bodyHtml = templateEngine.process("scholarship-document-verify.html", context);

            // Buat dan kirim email
            MimeMessage mimeMessage = javaMailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, StandardCharsets.UTF_8.name());
            helper.setTo(email);
            helper.setSubject("Pemberitahuan status verifikasi dokumen beasiswa");
            helper.setFrom(configAppProperties.mailFrom());
            helper.setText(bodyHtml, true);

            javaMailSender.send(mimeMessage);
        } catch (Exception e) {
            throw new MessagingException("Gagal mengirim email notifikasi dokumen beasiswa", e);
        }
    }
